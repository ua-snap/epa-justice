...
def create_new_record(new_id, name, region, country, lat, lon, alt_name):
    ...
    if alt_name == None:
        alt_name = np.nan
    # ignoring region dict here, since region is already spelled out in source df
    record = [new_id, name, alt_name, region, country, lat, lon, 0]
    return record
...
def insert_new_record(df, record):
    ...
    # new_df = df.append(row, ignore_index=True) # cannot use this after pandas 2.0!
    # use loc instead
    df.loc[len(df)] = row
    new_df = df.round(4)
    return new_df
